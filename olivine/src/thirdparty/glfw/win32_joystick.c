...
static DIOBJECTDATAFORMAT _glfwObjectDataFormats[] = {
  ...
  { &GUID_POV,
    DIJOFS_POV(0),
    DIDFT_POV | DIDFT_OPTIONAL | DIDFT_ANYINSTANCE,
    0 },
  { &GUID_POV,
    DIJOFS_POV(1),
    DIDFT_POV | DIDFT_OPTIONAL | DIDFT_ANYINSTANCE,
  ...
  { NULL,
    DIJOFS_BUTTON(0),
    DIDFT_BUTTON | DIDFT_OPTIONAL | DIDFT_ANYINSTANCE,
    0 },
  { NULL,
    DIJOFS_BUTTON(1),
    DIDFT_BUTTON | DIDFT_OPTIONAL | DIDFT_ANYINSTANCE,
...
//
static const char*
getDeviceDescription(const XINPUT_CAPABILITIES* xic)
{
  switch (xic->SubType) {
...
//
static int
compareJoystickObjects(const void* first, const void* second)
{
...
//
static GLFWbool
supportsXInput(const GUID* guid)
{
  UINT i, count = 0;
  ...
  for (i = 0; i < count; i++) {
    ...
    if ((INT)GetRawInputDeviceInfoA(
          ridl[i].hDevice, RIDI_DEVICEINFO, &rdi, &size) == -1) {
      continue;
    }
    ...
    if ((INT)GetRawInputDeviceInfoA(
          ridl[i].hDevice, RIDI_DEVICENAME, name, &size) == -1) {
      break;
    }
...
//
static void
closeJoystick(_GLFWjoystick* js)
{
  if (js->win32.device) {
...
deviceObjectCallback(const DIDEVICEOBJECTINSTANCEW* doi, void* user)
{
  ...
  if (DIDFT_GETTYPE(doi->dwType) & DIDFT_AXIS) {
    ...
    if (FAILED(IDirectInputDevice8_SetProperty(
          data->device, DIPROP_RANGE, &dipr.diph))) {
      return DIENUM_CONTINUE;
    }
...
//
static BOOL CALLBACK
deviceCallback(const DIDEVICEINSTANCE* di, void* user)
{
  int jid = 0;
  DIDEVCAPS dc;
  DIPROPDWORD dipd;
  ...
  if (FAILED(IDirectInput8_CreateDevice(
        _glfw.win32.dinput8.api, &di->guidInstance, &device, NULL))) {
  ...
  if (FAILED(
        IDirectInputDevice8_SetProperty(device, DIPROP_AXISMODE, &dipd.diph))) {
    _glfwInputError(GLFW_PLATFORM_ERROR,
  ...
  if (!WideCharToMultiByte(
        CP_UTF8, 0, di->tszInstanceName, -1, name, sizeof(name), NULL, NULL)) {
    _glfwInputError(GLFW_PLATFORM_ERROR,
...
//
void
_glfwTerminateJoysticksWin32(void)
{
  int jid;
...
//
void
_glfwDetectJoystickConnectionWin32(void)
{
  if (_glfw.win32.xinput.instance) {
    ...
    for (index = 0; index < XUSER_MAX_COUNT; index++) {
      ...
      // Generate a joystick GUID that matches the SDL 2.0.5+ one
      sprintf_s(
        guid, 33, "78696e707574%02x000000000000000000", xic.SubType & 0xff);
...
//
void
_glfwDetectJoystickDisconnectionWin32(void)
{
  int jid;
...
_glfwPlatformPollJoystick(_GLFWjoystick* js, int mode)
{
  if (js->win32.device) {
    ...
    result = IDirectInputDevice8_GetDeviceState(
      js->win32.device, sizeof(state), &state);
    ...
    for (i = 0; i < js->win32.objectCount; i++) {
      ...
      switch (js->win32.objects[i].type) {
        ...
        case _GLFW_TYPE_POV: {
          const int states[9] = { GLFW_HAT_UP,      GLFW_HAT_RIGHT_UP,
                                  GLFW_HAT_RIGHT,   GLFW_HAT_RIGHT_DOWN,
                                  GLFW_HAT_DOWN,    GLFW_HAT_LEFT_DOWN,
                                  GLFW_HAT_LEFT,    GLFW_HAT_LEFT_UP,
                                  GLFW_HAT_CENTERED };
...
_glfwPlatformUpdateGamepadGUID(char* guid)
{
  if (strcmp(guid + 20, "504944564944") == 0) {
    char original[33];
    strncpy_s(original, 33, guid, sizeof(original) - 1);
    sprintf_s(
      guid, 33, "03000000%.4s0000%.4s000000000000", original, original + 4);
  }
}
